...
class OptunaXGBScene(Scene):
    def construct(self):
        ...
        # Output
        divider = Line(LEFT * 6.8 + DOWN * 2.2, RIGHT * 6.8 + DOWN * 2.2,
                       stroke_color=C_DIM, stroke_width=0.8)
        self.play(Create(divider), run_time=0.4)

        result = VGroup(
            Text("Mejor configuracion XGBoost:", font_size=16, color=C_TEXT),
            Text("n_est=2103, depth=6, lr=0.011, subsample=0.50",
                 font_size=13, color=C_XGB),
            Text("CV MAE = 6.36 anios  |  Test MAE = 5.62 anios",
                 font_size=14, color=C_GOLD),
        )
        result.arrange(DOWN, buff=0.08, aligned_edge=LEFT)
        result.move_to(DOWN * 2.95)
