...
\title{Estimación de edad cerebral mediante\\integración multimodal de neuroimagen}
\subtitle{$\beta$-VAE + XGBoost sobre conectividad funcional y datos estructurales}
...
\begin{frame}{Autoencoders: comprimir para aprender}
  \begin{columns}[c]
    \begin{column}{0.48\textwidth}
      ...
      \vspace{0.2cm}
      {\small El modelo se entrena minimizando la diferencia entre entrada y reconstrucción:}
      \[
        \mathcal{L}_{\text{rec}}(\mathbf{x}, \tilde{\mathbf{x}})
      \]

      {\small Cada dato de entrada queda representado como un \textbf{punto} en el espacio latente.}
    \end{column}
    \begin{column}{0.50\textwidth}
      \centering
      \shorthandoff{>}
...
\begin{frame}{$\beta$-VAE: controlando el balance}
  ...
  \vspace{0.2cm}
  \begin{itemize}
    \item $\beta = 1 \rightarrow$ VAE estándar
    \item El valor óptimo de $\beta$, la dimensión latente y la arquitectura se determinan por \textbf{optimización automática} (ver Metodología)
    \item \textbf{Warmup:} $\beta$ crece linealmente desde 0 durante las primeras épocas para evitar el \textit{colapso posterior} (el encoder ignora la entrada)
    \item $\mathcal{L}_{\text{rec}}$ = MAE (error absoluto medio), más robusto que MSE ante valores atípicos
  \end{itemize}
\end{frame}

\begin{frame}{Árboles de decisión: partición del espacio}
  \begin{columns}[c]
    \begin{column}{0.48\textwidth}
      \centering
      \includegraphics[width=0.92\textwidth]{04_marco_teorico/images/fig6_regiones_finales.png}\\[2pt]
      {\scriptsize Regiones finales: cada rectángulo\\tiene una predicción constante.}
    \end{column}
    \begin{column}{0.48\textwidth}
      \centering
      \includegraphics[width=0.92\textwidth]{04_marco_teorico/images/fig7_punto_nuevo_real_vs_pred.png}\\[2pt]
      {\scriptsize Predicción de un nuevo punto:\\real $= 22{,}1$, predicho $= 20{,}1$.}
    \end{column}
  \end{columns}

  \vspace{0.15cm}
  \begin{itemize}
    \item Dividen recursivamente el espacio en regiones cada vez más homogéneas
    \item Criterio: minimizar la varianza en cada partición
  \end{itemize}
\end{frame}

\begin{frame}{Árboles de decisión: estructura y ensambles}
  \begin{columns}[c]
    \begin{column}{0.50\textwidth}
      \begin{itemize}
        \item Cada nodo interno: condición sobre una variable
        \item Cada hoja: valor de predicción constante (promedio)
      \end{itemize}

      \vspace{0.15cm}
      \begin{alertblock}{\small Limitación: sobreajuste}
        {\small Un solo árbol se adapta demasiado a los datos $\rightarrow$ se combina con \textbf{ensambles}.}
      \end{alertblock}

      \vspace{0.15cm}
      {\small \textbf{Ensambles de árboles:}}
      \begin{itemize}
        \item \textbf{Bagging} (ej: Random Forest): árboles en \highlight{paralelo}, promedio de predicciones
        \item \textbf{Boosting}: árboles \highlight{secuenciales}, cada uno corrige errores del anterior
      \end{itemize}

      \vspace{0.1cm}
      {\scriptsize \textbf{Idea clave:} muchos \textit{weak learners} (modelos simples) combinados producen un predictor fuerte (Schapire, 1990).}
    \end{column}
    \begin{column}{0.47\textwidth}
      \centering
      \includegraphics[width=\textwidth]{04_marco_teorico/images/fig2_arbol_completo.png}\\[3pt]
      {\scriptsize Árbol de regresión completo}
    \end{column}
  \end{columns}
\end{frame}

\begin{frame}{Gradient Boosting: ensamble secuencial}
  \begin{columns}[c]
    \begin{column}{0.52\textwidth}
      \begin{itemize}
        \item Técnica de \textbf{boosting}: árboles agregados secuencialmente
        \item Cada nuevo árbol corrige los \highlight{errores residuales} del modelo acumulado
        \item Avanza en la dirección que \textbf{reduce el gradiente} de la pérdida
        \item Predicción final: \textbf{suma ponderada} de todos los árboles
        \item Un \textit{learning rate} controla cuánto aporta cada corrección
      \end{itemize}
    \end{column}
...
\begin{frame}{XGBoost}
  \begin{columns}[c]
    \begin{column}{0.55\textwidth}
      \begin{itemize}
        \item Implementación optimizada de \textbf{Gradient Boosting} con árboles
        \item \highlight{Regularización explícita:} penaliza la complejidad de los árboles para evitar sobreajuste
        \item Búsqueda de divisiones por \textbf{cuantiles}
        \item Manejo nativo de \textbf{datos faltantes}
        \item Considerado uno de los \textbf{mejores modelos} para datos tabulares
      \end{itemize}

      \vspace{0.2cm}
      \begin{block}{En este trabajo}
        Recibe el vector de features (embeddings FC + volúmenes T1w) y predice la edad. Hiperparámetros optimizados con Optuna.
      \end{block}
    \end{column}
    \begin{column}{0.42\textwidth}
      \vspace{0.2cm}
      {\small \textbf{¿Por qué XGBoost?}}

      \vspace{0.3cm}
      \begin{itemize}
        \item[\checkmark] Regularización incluida
        \item[\checkmark] Eficiente en alta dimensionalidad
        \item[\checkmark] Proporciona \highlight{importancia de features} de forma directa (Gain)
        \item[\checkmark] Permite interpretar qué regiones cerebrales contribuyen a la predicción
      \end{itemize}
...
\begin{frame}{Objetivos del trabajo}
  \begin{block}{Objetivo general}
    Desarrollar y evaluar un pipeline de estimación de edad cerebral basado en la integración multimodal de datos funcionales y estructurales, analizar el impacto de variables demográficas y explorar la viabilidad como biomarcador.
  \end{block}

  \vspace{0.2cm}
  \textbf{Objetivos específicos:}
  \begin{enumerate}
    \item Implementar un $\beta$-VAE para reducción no lineal de las matrices de FC
    \item Integrar representaciones latentes (FC) con features T1w para predicción con XGBoost
    \item Optimizar el $\beta$-VAE con un \highlight{objetivo downstream} (predicción de edad)
    \item Cuantificar la contribución de cada modalidad (estudio de ablaciones)
    \item Comparar reducción no lineal ($\beta$-VAE) vs.\ lineal (PCA)
    \item Evaluar sobre la cohorte \highlight{latinoamericana} multicéntrica RedLaT
    \item Analizar el impacto de variables \highlight{demográficas} (educación, sitio) sobre la brain age gap
    \item Evaluar la viabilidad como \highlight{biomarcador} mediante entrenamiento exclusivo en controles sanos
  \end{enumerate}
\end{frame}
...
\begin{frame}{Optimización con objetivo \textit{downstream}}
  \begin{columns}[T]
    \begin{column}{0.46\textwidth}
      \begin{alertblock}{Enfoque tradicional}
        {\small Optimizar $\beta$-VAE para reconstruir bien\\
        $\rightarrow$ Podría capturar ruido\\irrelevante para la edad}
      \end{alertblock}
    \end{column}
    \begin{column}{0.46\textwidth}
      \begin{block}{Nuestro enfoque}
        {\small Optimizar $\beta$-VAE según qué tan bien\\predice la edad con sus embeddings\\
  ...
  \vspace{0.3cm}
  \textbf{Procedimiento (Optuna, 70 trials):}
  \begin{enumerate}
    \item Entrenar $\beta$-VAE con hiperparámetros propuestos
    \item Extraer embeddings $\rightarrow$ combinar con T1w
    \item Entrenar XGBoost (parámetros fijos) y evaluar MAE en 5-Fold CV
    \item Optuna busca la configuración que \highlight{minimice el MAE}
  \end{enumerate}
\end{frame}
...
\begin{frame}{CN-only + demográficos: hacia un biomarcador}
  ...
  \begin{tabular}{llrrrr}
    ...
    \midrule
    CN baseline & CN (test) & 53 & 6,09 & $-0{,}07$ \\
                & AD       & 422 & 6,97 & $+0{,}21$ \\
                & FTD      & 297 & 7,42 & $+4{,}01$ \\
    \midrule
...
\begin{frame}{Conclusiones}
  \begin{enumerate}
    \item Pipeline multimodal (FC + T1w) con MAE de \highlight{5,62 años}; al incluir educación y sitio como features con Optuna: \highlight{MAE 5,17}
    ...
    \vspace{0.18cm}
    \item El modelo CN-only + demográficos produce gaps consistentemente positivas en AD y FTD $\rightarrow$ \highlight{prometedor} como biomarcador con cohortes más amplias
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      ...
      \begin{itemize}
        ...
        \item Análisis demográfico ampliado (ingresos, ruralidad)
      \end{itemize}
    \end{column}
    \begin{column}{0.48\textwidth}
      \textbf{Modelos:}
