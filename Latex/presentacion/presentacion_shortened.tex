  ...
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      ...
      \begin{itemize}\setlength{\itemsep}{1pt}
        ...
        \item \highlight{MNI152}: plantilla promedio/de referencia + atlas \highlight{AAL116}
      \end{itemize}
    ...
    \begin{column}{0.45\textwidth}
      \centering
      \includegraphics[width=0.48\textwidth]{04_marco_teorico/images/t1_structural.png}\\[1pt]
      {\scriptsize T1w de un sujeto}\\[2pt]
      \includegraphics[width=0.48\textwidth]{04_marco_teorico/images/mni152+atlas.png}\\[1pt]
      {\scriptsize MNI152 (referencia) + AAL116}\\[2pt]
      \includegraphics[width=0.62\textwidth]{04_marco_teorico/images/bold_timeseries.png}\\[-2pt]
      {\scriptsize Serie temporal BOLD}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[shrink=5]{Conectividad funcional}
  ...
  \begin{columns}[c]
    \begin{column}{0.48\textwidth}
      \begin{itemize}
        \item \highlight{Correlación de Pearson} entre pares de regiones
        \item Matriz simétrica: $\mathrm{corr}(i,j)=\mathrm{corr}(j,i)$
        \item Diagonal $=1$; se descarta
...
\begin{frame}[shrink=10]{$\beta$-VAE: compresión inteligente de la conectividad}
  \begin{columns}[T]
    \begin{column}{0.50\textwidth}
      {\small \textbf{Objetivo:} comprimir FC de $6670 \to 64$ dimensiones sin perder la señal útil para edad.}

      \vspace{0.12cm}
      {\small \textbf{AE clásico:} cada entrada $\to$ un \textbf{punto fijo} en el latente.}

      \vspace{0.12cm}
      {\small \textbf{VAE:} cada entrada $\to$ una \highlight{distribución} $(\mu, \sigma)$. Introduce una parte probabilística controlada; el truco de reparametrización permite entrenarla.}

      \vspace{0.1cm}
      {\footnotesize \textbf{Pérdida de entrenamiento:}}
      \vspace{-0.12cm}
      \[
      \resizebox{0.6\linewidth}{!}{$
      \mathcal{L} =
      \underbrace{\mathcal{L}_{\mathrm{rec}}(x,\tilde{x})}_{\text{\scriptsize reconstrucción}}
      + \beta \cdot
      \underbrace{D_{\mathrm{KL}}(q(z|x)\,\|\,p(z))}_{\text{\scriptsize regularización latente}}
      $}
      \]
      \vspace{-0.18cm}

      \vspace{0.15cm}
      {\small \textbf{En este trabajo:}
      \begin{itemize}\setlength{\itemsep}{1pt}
        \item Para predecir edad se usa $\mu$ (determinístico).
        \item Búsqueda automática: $\beta = 0{,}057 \ll 1$.
        \item Lectura: preservar información fue más útil que regularizar fuerte.
      \end{itemize}}
    \end{column}
    \begin{column}{0.48\textwidth}
      ...
      \begin{tikzpicture}[scale=0.82, transform shape,
        ...
        \node[below=0.55cm of z, font=\tiny] (eps) {$\epsilon \sim \mathcal{N}(0,I)$};
        \node[above=0.05cm of z, font=\tiny] {$z = \mu + \sigma \odot \epsilon$};
...
\begin{frame}{División de los datos}
  ...
  \begin{tikzpicture}[
    ...
    \node[box, fill=unclight!50, below=1cm of tv, text width=3.4cm] (cv) {5-Fold CV\\4 entrenan + 1 valida\\(rotando)};
    \draw[arr] (all) -- (tv);
    \draw[arr] (all) -- (test);
    \draw[arr] (tv) -- (cv);
  \end{tikzpicture}
...
\begin{frame}[shrink=2]{Optimización en dos etapas}
  \begin{block}{\small Optuna: búsqueda automática de hiperparámetros}
    {\footnotesize Se prueban configuraciones posibles (por ej.\ $\beta$, dimensión latente, profundidad de árboles) y se eligen las que reducen el \highlight{MAE de edad} en validación cruzada. La optimización está orientada a la tarea final, no solo a reconstruir FC.}
  \end{block}
  ...
  \begin{tikzpicture}[scale=0.8, transform shape,
    ...
    \node[stage, fill=uncblue!15] (e1) at (0, 0) {\textbf{Etapa 1}\\Buscar $\beta$-VAE};
    \node[stage, fill=uncblue!15] (e2) at (5.5, 0) {\textbf{Etapa 2}\\Buscar XGBoost};
    \node[stage, fill=uncaccent!15] (e3) at (11, 0) {\textbf{Etapa 3}\\Modelo final};

    \node[optstep] (vae1) at (0, -1.8) {Entrenar $\beta$-VAE\\con configuración propuesta};
    \node[frozen] (xgb1) at (0, -3.2) {XGBoost\\(parámetros fijos)};
    \node[lbl] at (0, -4) {$\times$ 70 trials, 5-fold CV};

    \node[frozen] (vae2) at (5.5, -1.8) {$\beta$-VAE fijo\\(mejor de Etapa 1)};
    \node[optstep] (xgb2) at (5.5, -3.2) {Entrenar XGBoost\\con configuración propuesta};
  ...
  \vspace{0.08cm}
  {\scriptsize En cada trial se reutilizan los mismos 5 folds de trainval y se reporta el \highlight{MAE promedio} de validación.\\
  \textbf{Config.\ óptima:} $\beta = 0{,}057$; encoder $6670 \to 512 \to 64$; ELU; warmup 73. Vector final: \highlight{180 features}.}
\end{frame}

\begin{frame}{Pipeline: resumen animado}
  \centering
  \includegraphics[width=0.75\linewidth, height=0.62\textheight, keepaspectratio]{presentacion/07_pipeline_animado/pipeline_poster.png}
...
\begin{frame}[shrink=8]{Resultado del modelo principal}{\color{uncyellow}\scriptsize $\blacktriangleright$ Objetivo 1: Predicción de edad cerebral}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \centering
      \begin{minipage}[t]{\linewidth}
        \centering
        \includegraphics[width=0.86\linewidth]{06_resultados_discusion/images/xgb_scatter.png}\\[1pt]
    ...
    \begin{column}{0.50\textwidth}
      ...
      \vspace{0.12cm}
      \centering
      {\scriptsize \textbf{Contexto literatura} (MAE años; orientativo según $N$ y pipeline)}\\[2pt]
      \resizebox{\linewidth}{!}{%
      \begin{tabular}{@{}l l r r@{}}
        \toprule
        \textbf{Estudio} & \textbf{Modalidad / modelo} & $\boldsymbol{N}$ & \textbf{MAE} \\
        \midrule
        Franke et al.\ (2010) & T1w / RVM + PCA & 650 & 5,00 \\
        Liem et al.\ (2017) & T1w + FC / stacking & 2\,354 & 4,29 \\
        Gao et al.\ (2023) & FC / GNN (AAL) & 471 & 5,92 \\
        Bashyam et al.\ (2020) & T1w / 2D-CNN & 14\,468 & $\sim$3,7 \\
        Dufumier et al.\ (2024) & T1w + FC / M-AVAE & 5\,330 & 2,77 \\
        \midrule
        \rowcolor{unclight}
        \textbf{Este trabajo} & \textbf{FC + T1w / $\beta$-VAE + XGB} & \textbf{1\,245} & \textbf{5,62} \\
        \bottomrule
      \end{tabular}}
    \end{column}
  \end{columns}
...
\begin{frame}{Estudio de ablaciones}{\color{uncyellow}\scriptsize $\blacktriangleright$ Objetivo 1: Predicción de edad cerebral}
  ...
  \begin{tabular}{lrrrr}
    \toprule
    \textbf{Features} & \textbf{MAE} & \textbf{RMSE} & $\boldsymbol{R^2}$ & \textbf{Pearson} $r$ \\
    \midrule
...
\begin{frame}[shrink=2]{¿Importa cómo se comprime la conectividad?}{\color{uncyellow}\scriptsize $\blacktriangleright$ Objetivo 1: Predicción de edad cerebral}
  \begingroup
  \renewcommand{\arraystretch}{0.92}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      {\footnotesize \textbf{FC crudo vs.\ comprimido}}
      ...
      \vspace{0.18cm}
      {\footnotesize \highlight{97,3\%} menos features y mejor MAE.\\
      La \highlight{reducción} es lo clave, no si es lineal o no lineal.}
    \end{column}
    \begin{column}{0.50\textwidth}
      {\footnotesize \textbf{PCA vs.\ $\beta$-VAE} (misma dim.\ 64 + T1w):}
      ...
      \vspace{0.18cm}
      {\footnotesize Rendimiento \highlight{prácticamente idéntico}.\\
      XGBoost captura no linealidades aun desde PCA.\\[3pt]
      }
    \end{column}
  \end{columns}
  \endgroup
  ...
  \vspace{0.28cm}
  \centering
  {\scriptsize \textbf{Comparación de regresores} (mismas 180 features; Optuna 100 trials; métricas en test)}\\[0.14cm]
  {\scriptsize
  \begingroup
  \renewcommand{\arraystretch}{0.92}
  \begin{tabular}{lrrrr}
    \toprule
    \textbf{Regresor} & \textbf{MAE} & \textbf{RMSE} & $\boldsymbol{R^2}$ & \textbf{Pearson} $r$ \\
    \midrule
    Ridge & 5,75 & 7,25 & 0,403 & 0,639 \\
    SVR (RBF) & 5,65 & 7,02 & 0,441 & 0,664 \\
    \rowcolor{unclight}
    \textbf{XGBoost} & \textbf{5,62} & \textbf{7,20} & \textbf{0,411} & \textbf{0,644} \\
    \bottomrule
  \end{tabular}
  \endgroup}\\[0.12cm]
  {\scriptsize Tres familias distintas $\rightarrow$ resultados \highlight{prácticamente iguales}. Pesa más la \textbf{representación} que el regresor.}
...
\begin{frame}{Variables demográficas: educación}{\color{uncyellow}\scriptsize $\blacktriangleright$ Objetivo 2: Variables sociodemográficas}
  \begin{block}{Variable: \texttt{cog\_ed} --- años de educación formal (disponible 74,9\%)}
    {\small Pregunta: ¿a más educación, la gap tiende a subir o bajar?\\
    \textbf{Gap} = edad predicha $-$ edad real; positiva $\rightarrow$ más viejo de lo esperado.}
  \end{block}
  ...
  \begin{tabular}{lrrl}
    \toprule
    \textbf{Grupo} & \textbf{$r$ educación--gap} & $\boldsymbol{p}$ & \textbf{Interpretación} \\
    \midrule
...
\begin{frame}{Variables demográficas: sitio y escáner}{\color{uncyellow}\scriptsize $\blacktriangleright$ Objetivo 2: Variables sociodemográficas}
  \begin{block}{El sitio de adquisición es el mayor confundidor}
    {\small ANOVA: compara todos los centros a la vez. $p < 0{,}001$ $\rightarrow$ las gaps cambian según el sitio.}
  \end{block}
...
\begin{frame}{Demográficos como features: optimización con Optuna}{\color{uncyellow}\scriptsize $\blacktriangleright$ Objetivo 2: Variables sociodemográficas}
  ...
  \begin{tabular}{lrrrr}
    \toprule
    \textbf{Configuración} & $\boldsymbol{n_{\text{test}}}$ & \textbf{MAE} & $\boldsymbol{R^2}$ & \textbf{Pearson} $r$ \\
    \midrule
  ...
  \vspace{0.25cm}
  \begin{block}{Lectura predictiva}
...
\begin{frame}{CN-only + demográficos: hacia un biomarcador}{\color{uncyellow}\scriptsize $\blacktriangleright$ Objetivo 3: Biomarcador de envejecimiento acelerado}
  ...
  Resultado \highlight{prometedor}, pero no concluyente sin más CN y mejor control multi-sitio.}
...
\begin{frame}{Discusión}
  \begin{itemize}
    \item Rendimiento dentro del rango esperable para cohortes comparables en tamaño y modalidad

    \vspace{0.3cm}
    \item Trabajos con MAE $< 3$ suelen usar cohortes \textbf{mucho mayores} y más homogéneas

    \vspace{0.3cm}
    \item RedLaT aporta una cohorte \highlight{latinoamericana multicéntrica}, poco representada en la literatura

    \vspace{0.3cm}
    \item $R^2 = 0{,}41$: queda varianza por explicar $\rightarrow$ edad cerebral multifactorial
  \end{itemize}
\end{frame}

\begin{frame}[shrink=4]{Limitaciones, próximos pasos y cierre}
  \begin{columns}[T]
    \begin{column}{0.50\textwidth}
      \textbf{Limitación $\rightarrow$ próximo paso}
      \vspace{0.05cm}
      {\small
      \begin{itemize}\setlength{\itemsep}{2pt}
        \item Cohorte modesta, sobre todo CN $\rightarrow$ más sujetos control
        \item Efectos de sitio $\rightarrow$ armonización ComBat / validación por sitio
        \item Atlas AAL grueso $\rightarrow$ atlas de mayor resolución
        \item Modelos modulares $\rightarrow$ comparar con GNN / 3D CNN
        \item Búsqueda VAE acotada $\rightarrow$ más trials y espacio ampliado
      \end{itemize}}
    \end{column}
    \begin{column}{0.47\textwidth}
      \textbf{Conclusiones por objetivo}
      \vspace{0.05cm}
      {\small
      \begin{enumerate}\setlength{\itemsep}{5pt}
        \item \textbf{Predicción:} FC + T1w es viable; con demográficos alcanza \highlight{MAE 5,17}.
        \item \textbf{Sociodemográficos:} educación y sitio modulan la gap y ayudan a calibrar la predicción.
        \item \textbf{Biomarcador:} CN-only + demográficos es prometedor, pero requiere más CN y control multi-sitio.
      \end{enumerate}}
    \end{column}
  \end{columns}
\end{frame}
