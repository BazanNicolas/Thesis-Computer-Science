...
\begin{frame}[shrink=2]{Optimización en dos etapas}
  \begin{block}{\small Enfoque \textit{downstream}: optimización orientada a la tarea}
    {\footnotesize \textbf{Entrenamiento del $\beta$-VAE:} no supervisado sobre FC con pérdida ELBO ($\mathcal{L}_{\text{rec}} + \beta \cdot D_{KL}$).\\
    \textbf{Selección con Optuna:} los hiperparámetros (incl.\ $\beta$) se eligen por el \highlight{MAE de edad} en validación cruzada con XGBoost, no por reconstrucción ni por ELBO aislados. No se fija a priori el balance reconstrucción vs.\ regularización del latente: la búsqueda descubre qué configuración sirve mejor para la tarea downstream.}
  \end{block}
...
\begin{frame}{Variables demográficas: educación}{\color{uncyellow}\scriptsize $\blacktriangleright$ Objetivo 2: Variables sociodemográficas}
  ...
  \begin{tabular}{lrrl}
    \toprule
    \textbf{Grupo} & \textbf{Pearson} $r$ & $\boldsymbol{p}$ & \textbf{Interpretación} \\
    \midrule
